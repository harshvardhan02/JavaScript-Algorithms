# JavaScript Algorithms

![Logo](https://cdn-icons-png.flaticon.com/128/5968/5968292.png)   ![Logo](https://cdn-icons-png.flaticon.com/128/6993/6993261.png)    ![Logo](https://cdn-icons-png.flaticon.com/128/9716/9716179.png) 

# Table of Contents 📚
 
- [➊ Sorting Algorithms](#-sorting-algorithms)
  - [⭐ Quick Sort](#-quick-sort)
  - [⭐ Merge Sort](#-merge-sort)
  - [⭐ Heap Sort](#-heap-sort)  
  - [⭐ Bubble Sort](#-bubble-sort)  
  - [⭐ Insertion Sort](#-insertion-sort)  
  - [⭐ Selection Sort](#-selection-sort)  
- [➋ Searching Algorithms](#-searching-algorithms)  
  - [⭐ Binary Search](#-binary-search)  
  - [⭐ Linear Search](#-linear-search)  
- [➌ Dynamic Programming (DP)](#-dynamic-programming-dp)  
  - [⭐ Fibonacci Sequence](#-fibonacci-sequence)  
  - [⭐ Knapsack Problem](#-knapsack-problem)  
  - [⭐ Longest Common Subsequence](#-longest-common-subsequence)  
  - [⭐ Longest Increasing Subsequence](#-longest-increasing-subsequence)  
  - [⭐ Coin Change](#-coin-change)  
  - [⭐ Edit Distance](#-edit-distance)  
  - [⭐ House Robber](#-house-robber)  
  - [⭐ Palindromic Substring](#-palindromic-substring)  
  - [⭐ Partition Equal Subset Sum](#-partition-equal-subset-sum)  
- [➍ Greedy Algorithms](#-greedy-algorithms)  
  - [⭐ Activity Selection](#-activity-selection)  
  - [⭐ Huffman Coding](#-huffman-coding)  
  - [⭐ Job Sequencing](#-job-sequencing)  
  - [⭐ Fractional Knapsack](#-fractional-knapsack)  
  - [⭐ Dijkstra’s Algorithm](#-dijkstras-algorithm)  
- [➎ Graph Algorithms](#-graph-algorithms)  
  - [⭐ Depth-First Search (DFS)](#-depth-first-search-dfs)  
  - [⭐ Breadth-First Search (BFS)](#-breadth-first-search-bfs)  
  - [⭐ Dijkstra’s Algorithm](#-dijkstras-algorithm-1)  
  - [⭐ Bellman-Ford Algorithm](#-bellman-ford-algorithm)  
  - [⭐ Kruskal’s Algorithm](#-kruskals-algorithm)  
  - [⭐ Prim’s Algorithm](#-prims-algorithm)  
  - [⭐ Topological Sort](#-topological-sort)  
  - [⭐ Floyd-Warshall Algorithm](#-floyd-warshall-algorithm)  
  - [⭐ Union-Find Algorithm](#-union-find-algorithm)  
- [➏ Backtracking Algorithms](#-backtracking-algorithms)  
  - [⭐ N-Queens Problem](#-n-queens-problem)  
  - [⭐ Sudoku Solver](#-sudoku-solver)  
  - [⭐ Subset Sum Problem](#-subset-sum-problem)  
  - [⭐ Rat in a Maze](#-rat-in-a-maze)  
  - [⭐ Word Search](#-word-search)  
  - [⭐ Permutations and Combinations](#-permutations-and-combinations)  
- [➐ Divide and Conquer](#-divide-and-conquer)  
  - [⭐ Merge Sort](#-merge-sort-1)  
  - [⭐ Quick Sort](#-quick-sort-1)  
  - [⭐ Binary Search](#-binary-search-1)  
  - [⭐ Maximum Subarray Problem](#-maximum-subarray-problem)  
- [➑ String Algorithms](#-string-algorithms)  
  - [⭐ KMP Algorithm](#-kmp-algorithm)  
  - [⭐ Rabin-Karp Algorithm](#-rabin-karp-algorithm)  
  - [⭐ Longest Palindromic Substring](#-longest-palindromic-substring)  
  - [⭐ Z Algorithm](#-z-algorithm)  
  - [⭐ Trie Data Structure](#-trie-data-structure)  
- [➒ Mathematical Algorithms](#-mathematical-algorithms)  
  - [⭐ Euclidean GCD Algorithm](#-euclidean-gcd-algorithm)  
  - [⭐ Modular Exponentiation](#-modular-exponentiation)  
  - [⭐ Fibonacci Numbers using Matrix Exponentiation](#-fibonacci-numbers-using-matrix-exponentiation)  
- [➓ Tree and Binary Tree Algorithms](#-tree-and-binary-tree-algorithms)  
  - [⭐ Binary Tree Traversals](#-binary-tree-traversals)  
  - [⭐ Lowest Common Ancestor (LCA)](#-lowest-common-ancestor-lca)  
  - [⭐ Binary Search Tree (BST) Operations](#-binary-search-tree-bst-operations)  
  - [⭐ Diameter of a Binary Tree](#-diameter-of-a-binary-tree)  
  - [⭐ Level Order Traversal](#-level-order-traversal)  
  - [⭐ Serialise and Deserialise Binary Tree](#-serialise-and-deserialise-binary-tree)  
  - [⭐ Maximum Path Sum](#-maximum-path-sum)  
  - [⭐️ Traversals & Views of Trees](#-traversals--views-of-trees)  
- [⓫ Linked List Algorithms](#-linked-list-algorithms)  
  - [⭐ Reverse a Linked List](#-reverse-a-linked-list)  
  - [⭐ Detect Cycle in a Linked List](#-detect-cycle-in-a-linked-list)  
  - [⭐ Merge Two Sorted Linked Lists](#-merge-two-sorted-linked-lists)  
  - [⭐ Intersection Point of Two Linked Lists](#-intersection-point-of-two-linked-lists)  
- [⓬ Heap/ Priority Queue Algorithms](#-heap-priority-queue-algorithms)  
  - [⭐ Kth Largest Element](#-kth-largest-element)  
  - [⭐ Merge K Sorted Lists](#-merge-k-sorted-lists)  
  - [⭐ Find Median from Data Stream](#-find-median-from-data-stream)  
- [⓭ Bit Manipulation Algorithms](#-bit-manipulation-algorithms)  
  - [⭐ Power of 2](#-power-of-2)  
  - [⭐ Count Set Bits](#-count-set-bits)  
  - [⭐ Find Non-Repeating Element](#-find-non-repeating-element)  
  - [⭐ XOR Problems](#-xor-problems)  
- [⓮ Miscellaneous](#-miscellaneous)  
  - [⭐ Sliding Window Technique](#-sliding-window-technique)  
  - [⭐ Two Pointer Technique](#-two-pointer-technique)  
  - [⭐ Tortoise and Hare Algorithm](#-tortoise-and-hare-algorithm)  
  - [⭐ Reservoir Sampling](#-reservoir-sampling)  
  - [⭐ Monotonic Stack](#-monotonic-stack)  

---

## ➊ Sorting Algorithms  
### ⭐ Quick Sort  
*(Description/implementation details here)*  
### ⭐ Merge Sort  
*(Description/implementation details here)*  
### ⭐ Heap Sort  
*(Description/implementation details here)*  
### ⭐ Bubble Sort  
*(Description/implementation details here)*  
### ⭐ Insertion Sort  
*(Description/implementation details here)*  
### ⭐ Selection Sort  
*(Description/implementation details here)*  

---

*(Repeat the same structure for all other sections with subsections...)*  
